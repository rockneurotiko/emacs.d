...
(defvar plantuml-jar-path "/opt/plantuml/plantuml.jar")
...
(defun plantuml-init ()
  ...
  (unless (file-exists-p "/opt/plantuml/plantuml.jar")
    (error "Could not find plantuml.jar at %s" "/opt/plantuml/plantuml.jar"))
  (with-temp-buffer
    (shell-command (concat "java -jar "
                           (shell-quote-argument "/opt/plantuml/plantuml.jar")
...
(defun plantuml-render-string (string unicode)
  ...
  (let ((in-file (make-temp-file "plantuml"))
    ...
    (let ((out-file (concat (file-name-sans-extension in-file) out-ext))
          (command (format "java -jar %s %s %s" "/opt/plantuml/plantuml.jar" out-opt
                           in-file)))
